...
class TSP(OptProblem):
    ...
    def max_action(self, state: list[int], tabu_list: list[tuple[int, int]] = []) -> tuple[tuple[int, int], float]:
        ...
        for a in self.actions(state):
            ...
            # Asegurarnos de que i < j (siguiendo la regla de que la acción (i, j) debe tener i < j)
            if i >= j:
                continue  # Ignoramos acciones que no cumplen con la condición
        
            # Comprobamos que la acción no esté en la lista tabú
            if a in tabu_list:
                continue  # Si la acción está en la lista tabú, la ignoramos
